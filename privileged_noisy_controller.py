...
class CBF(BaseController):
    ...
    def __call__(self, agent: Car, world: World):
        ...
        if len(world.agents) > 1:
            ...
            ## IMPLEMENT NOISE HERE FOR OBSTACLE
            random_bias = np.random.normal(0.125, 0.05,6)
            # Ensure the number is between 0 and 1, very likely given mu and sigma
            for elem in random_bias:
                elem = max(0, min(1, elem))


            obs_s     = obs_dyn.s + random_bias[0]
            obs_d     = obs_dyn.d + random_bias[1]
            obs_mu    = obs_dyn.mu + random_bias[2]
            obs_v     = obs_dyn.speed + random_bias[3]
            obs_delta = obs_dyn.steering + random_bias[4]
            obs_kappa = obs_dyn.kappa + random_bias[5]
